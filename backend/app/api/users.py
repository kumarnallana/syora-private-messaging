...
def friendship_out(item: Friendship) -> dict: return {"id": str(item.id), "from": str(
    item.requester_id), "to": str(item.addressee_id), "status": item.status.value.lower()}
...
async def search_users(q: str = Query(min_length=1, max_length=80), limit: int = Query(20, ge=1, le=30), user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    handle_escaped = handle.replace("\\", "\\\\").replace(
        "%", "\\%").replace("_", "\\_")
    blocked_pair = exists(select(Friendship.id).where(
        Friendship.status == FriendshipStatus.BLOCKED, pair_clause(user.id, User.id)))
    rank = case((User.username == handle, 0), (User.username.ilike(
        f"{handle_escaped}%", escape="\\"), 1), else_=2)
...
async def contacts(user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    rels = (await db.scalars(select(Friendship).where(Friendship.status == FriendshipStatus.ACCEPTED, or_(Friendship.requester_id == user.id, Friendship.addressee_id == user.id)))).all()
    ids = {r.addressee_id if r.requester_id ==
           user.id else r.requester_id for r in rels}
    ids.discard(user.id)
    people = (await db.scalars(select(User).where(User.id.in_(ids)))).all() if ids else []
    return {"users": [await user_out(db, x, user.id) for x in people], "friendships": [friendship_out(x) for x in rels]}


@router.get("/api/friend-requests")
async def friend_requests(user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    rels = (await db.scalars(select(Friendship).where(Friendship.status == FriendshipStatus.PENDING, or_(Friendship.requester_id == user.id, Friendship.addressee_id == user.id)))).all()
    ids = {x.requester_id for x in rels} | {x.addressee_id for x in rels}
    ids.discard(user.id)
    people = (await db.scalars(select(User).where(User.id.in_(ids)))).all() if ids else []
    return {"users": [await user_out(db, x, user.id) for x in people], "friendships": [friendship_out(x) for x in rels]}
...
async def request_friend(body: FriendRequestIn, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    if body.user_id == user.id:
        raise api_error(422, "FRIEND_SELF",
    ...
    if existing and existing.status == FriendshipStatus.DECLINED:
        ...
        await db.commit()
        return friendship_out(existing)
    if existing:
        raise api_error(409, "RELATIONSHIP_EXISTS",
                        "A relationship already exists with this user.")
    item = Friendship(requester_id=user.id,
                      addressee_id=body.user_id, status=FriendshipStatus.PENDING)
    db.add(item)
    await db.commit()
    await db.refresh(item)
    return friendship_out(item)
...
async def respond_friend(request_id: uuid.UUID, body: FriendResponseIn, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not item:
        raise api_error(404, "REQUEST_NOT_FOUND", "Friend request not found.")
    item.status = FriendshipStatus.ACCEPTED if body.action == "accept" else FriendshipStatus.DECLINED
    await db.commit()
    return friendship_out(item)
...
async def remove_contact(other_id: uuid.UUID, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not item:
        raise api_error(404, "CONTACT_NOT_FOUND", "Contact not found.")
...
async def block_user(other_id: uuid.UUID, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    if other_id == user.id:
        raise api_error(422, "BLOCK_SELF", "You cannot block yourself.")
    if not await db.get(User, other_id):
        raise api_error(404, "USER_NOT_FOUND", "User not found.")
    item = await relationship(db, user.id, other_id)
    if item:
        item.requester_id = user.id
        item.addressee_id = other_id
        item.status = FriendshipStatus.BLOCKED
    else:
        item = Friendship(requester_id=user.id,
                          addressee_id=other_id, status=FriendshipStatus.BLOCKED)
        db.add(item)
    await db.commit()
    return friendship_out(item)
...
async def unblock_user(other_id: uuid.UUID, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not item:
        raise api_error(404, "BLOCK_NOT_FOUND", "Blocked user not found.")
    await db.delete(item)
    await db.commit()


@router.get("/api/profile")
async def profile(user: User = Depends(current_user), db: AsyncSession = Depends(
    get_db)): return await user_out(db, user, user.id)
...
async def update_profile(body: ProfileIn, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    if body.username is not None and body.username != user.username:
        if await db.scalar(select(User.id).where(User.username == body.username, User.id != user.id)):
            raise api_error(409, "USERNAME_EXISTS",
    ...
    if body.avatar_key is not None:
        if not body.avatar_key.startswith(f"users/{user.id}/avatar/"):
            raise api_error(422, "AVATAR_INVALID", "Avatar upload is invalid.")
        if not body.avatar_mime or not body.avatar_size:
            raise api_error(422, "AVATAR_INVALID",
                            "Avatar metadata is required.")
        from app.services.media import r2
        r2.validate_metadata("avatar", body.avatar_mime, body.avatar_size)
        await asyncio.to_thread(r2.verify_object, body.avatar_key, body.avatar_mime, body.avatar_size)
...
async def get_preferences(user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    values = await preferences_out(db, user.id)
    blocked = (await db.scalars(select(Friendship.addressee_id).where(Friendship.requester_id == user.id, Friendship.status == FriendshipStatus.BLOCKED))).all()
    people = (await db.scalars(select(User).where(User.id.in_(blocked)))).all() if blocked else []
    values["blocked"] = [str(x) for x in blocked]
    values["blockedUsers"] = [await user_out(db, x, user.id) for x in people]
    await db.commit()
    return values
...
async def update_preferences(body: PreferencesIn, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    return await preferences_out(db, user.id)
