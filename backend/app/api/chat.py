...
def direct_key(a:uuid.UUID,b:uuid.UUID)->str:return ":".join(sorted((str(a),str(b))))
async def conversation_out(db:AsyncSession,conversation:Conversation,participant:ConversationParticipant,user_id:uuid.UUID)->dict:
    other_id=await db.scalar(select(ConversationParticipant.user_id).where(ConversationParticipant.conversation_id==conversation.id,ConversationParticipant.user_id!=user_id));other=await db.get(User,other_id) if other_id else None
...
async def create_direct(body:DirectConversationIn,user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    if body.user_id==user.id:raise api_error(422,"CONVERSATION_SELF","You cannot start a conversation with yourself.")
    ...
    if not rel:raise api_error(403,"FRIENDSHIP_REQUIRED","An accepted friendship is required.")
    if await blocked(db,user.id,body.user_id):raise api_error(403,"CONTACT_BLOCKED","This conversation is unavailable.")
    key=direct_key(user.id,body.user_id);existing=await db.scalar(select(Conversation).where(Conversation.direct_key==key))
    if existing:return await conversation_out(db,existing,await require_participant(existing.id,user,db),user.id)
    item=Conversation(type=ConversationType.DIRECT,direct_key=key);db.add(item);await db.flush();db.add_all([ConversationParticipant(conversation_id=item.id,user_id=user.id),ConversationParticipant(conversation_id=item.id,user_id=body.user_id)]);await db.commit();participant=await require_participant(item.id,user,db);data=await conversation_out(db,item,participant,user.id);await emit_user(body.user_id,"conversation:update",{"conversationId":str(item.id)});return data
...
async def get_conversation(conversation_id:uuid.UUID,user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    participant=await require_participant(conversation_id,user,db);item=await db.get(Conversation,conversation_id);return await conversation_out(db,item,participant,user.id)
...
async def set_conversation_preferences(conversation_id:uuid.UUID,body:ConversationPreferenceIn,user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    ...
    if body.pinned is not None:participant.pinned=body.pinned
    if body.muted is not None:participant.muted=body.muted
    await db.commit();return {"pinned":participant.pinned,"muted":participant.muted}
...
async def list_messages(conversation_id:uuid.UUID,before:datetime|None=None,limit:int=Query(30,ge=1,le=100),user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    ...
    query=select(Message).where(Message.conversation_id==conversation_id,~Message.id.in_(select(MessageVisibility.message_id).where(MessageVisibility.user_id==user.id)))
    if before:query=query.where(Message.created_at<before)
    rows=(await db.scalars(query.order_by(Message.created_at.desc()).limit(limit))).all();rows.reverse();return {"messages":[await message_out(db,x,user.id) for x in rows],"nextCursor":rows[0].created_at.isoformat() if len(rows)==limit else None}
...
async def send_message(conversation_id:uuid.UUID,body:MessageIn,user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    await require_participant(conversation_id,user,db);participant_ids=(await db.scalars(select(ConversationParticipant.user_id).where(ConversationParticipant.conversation_id==conversation_id))).all()
    other_ids=[x for x in participant_ids if x!=user.id]
    if any([await blocked(db,user.id,x) for x in other_ids]):raise api_error(403,"CONTACT_BLOCKED","Messages cannot be sent in this conversation.")
    if not body.text and not body.attachment:raise api_error(422,"MESSAGE_EMPTY","Add text or an attachment.")
    if body.reply_to and not await db.scalar(select(Message.id).where(Message.id==body.reply_to,Message.conversation_id==conversation_id)):raise api_error(422,"REPLY_INVALID","The reply target is not in this conversation.")
    ...
    for other_id in other_ids:await emit_user(other_id,"conversation:update",{"conversationId":str(conversation_id)})
    return data
...
async def delete_message(message_id:uuid.UUID,body:DeleteMessageIn,user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    ...
    if body.mode=="me":
        if not await db.get(MessageVisibility,{"message_id":item.id,"user_id":user.id}):db.add(MessageVisibility(message_id=item.id,user_id=user.id))
    else:
...
async def mark_message_read(message_id:uuid.UUID,user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    ...
    if not item:raise api_error(404,"MESSAGE_NOT_FOUND","Message not found.")
    await mark_read_internal(item,user,db);return {"read":True}
...
async def mark_conversation_read(conversation_id:uuid.UUID,user:User=Depends(current_user),db:AsyncSession=Depends(get_db)):
    await require_participant(conversation_id,user,db);items=(await db.scalars(select(Message).join(MessageReceipt,MessageReceipt.message_id==Message.id).where(Message.conversation_id==conversation_id,MessageReceipt.user_id==user.id,MessageReceipt.read_at.is_(None)))).all()
    for item in items:
