...
class UserPreference(Base):
    ...
    admin_login_notifications:Mapped[bool]=mapped_column(Boolean,default=True)
    admin_message_notifications:Mapped[bool]=mapped_column(Boolean,default=True)
    admin_message_preview:Mapped[bool]=mapped_column(Boolean,default=True)
    admin_push_enabled:Mapped[bool]=mapped_column(Boolean,default=False)

class PushSubscription(Base):
    __tablename__="push_subscriptions"
    id:Mapped[uuid.UUID]=mapped_column(UUID(as_uuid=True),primary_key=True,default=uuid.uuid4)
    user_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("users.id",ondelete="CASCADE"),index=True)
    session_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("refresh_sessions.id",ondelete="CASCADE"),index=True)
