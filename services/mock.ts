...
export class MockServices implements Services {
 ...
 async updateProfile(values:Partial<Pick<User,'name'|'about'|'avatar'>>){const id=this.identity();if(values.name!==undefined&&(!values.name.trim()||values.name.trim().length>80))throw new Error('Use a display name between 1 and 80 characters.');const next={...values,...(values.name!==undefined?{name:values.name.trim()}:{})};this.update({users:this.state.users.map(u=>u.id===id?{...u,...next}:u)});}
