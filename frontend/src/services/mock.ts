...
await pause();return this.signIn('demo');}
 ...
 async getAdminMetrics():Promise<AdminMetrics>{const me=this.user(this.identity());if(me.role!=="admin")throw new Error("Administrator access is required.");return {signedInUsers:1,people:[{userId:me.id,displayName:me.name,username:me.username,sessionCount:1,lastActiveAt:new Date().toISOString(),isCurrentUser:true,canRevoke:false}]};}
