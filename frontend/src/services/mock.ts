...
await pause();return this.signIn('demo');}
 ...
 async searchUsers(query:string){const value=query.trim().replace(/^@/,'').toLowerCase();return this.state.users.filter(user=>user.id!==this.state.currentUserId&&(user.username.startsWith(value)||user.name.toLowerCase().includes(value))).slice(0,20)}
