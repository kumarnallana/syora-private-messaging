...
export function Contacts(){
 ...
 async function act(work:()=>Promise<unknown>){try{setError('');await work()}catch(e){setError((e as Error).message)}}
 function actions(user:User){const rel=relationship(user.id);const accepted=rel?.status==='accepted';const pending=rel?.status==='pending';const blocked=preferences.blocked.includes(user.id);return <div className="row-actions">{accepted&&!blocked&&<button className="button secondary small" onClick={()=>open(user.id)}><MessageCircle size={16}/> Message</button>}{!rel&&!blocked&&<button className="button secondary small" onClick={()=>void act(()=>services.request(user.id))}><UserPlus size={16}/> Add</button>}{pending&&<span className="status-pill">{rel.from===me!.id?'Request sent':'Awaiting response'}</span>}{accepted&&!blocked&&<IconButton label={`Remove ${user.name} from friends`} onClick={()=>void act(()=>services.remove(user.id))}><UserMinus size={18}/></IconButton>}{blocked&&<button className="button secondary small" onClick={()=>void act(()=>services.block(user.id))}><Ban size={16}/> Unblock</button>}</div>}
