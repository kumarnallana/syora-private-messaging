...
export function Status() {
  ...
  useEffect(() => {
    const status = active === null ? undefined : activeStatuses[active];
    if (!status || status.userId === me!.id || status.viewedBy.includes(me!.id)) return;
    void services.view(status.id).catch(cause => setError(cause instanceof Error ? cause.message : 'This status could not be opened.'));
  }, [active, activeStatuses, me, services]);
