...
export function Status() {
  ...
  const activeStatus = active === null ? undefined : activeStatuses[active];

  useEffect(() => {
    if (!activeStatus || activeStatus.userId === me!.id || activeStatus.viewedBy.includes(me!.id)) return;
    void services.view(activeStatus.id).catch(cause => setError(cause instanceof Error ? cause.message : 'This status could not be opened.'));
  }, [activeStatus?.id, activeStatus?.userId, activeStatus?.viewedBy, me, services]);
