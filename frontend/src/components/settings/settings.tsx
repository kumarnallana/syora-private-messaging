...
function AdminSessionCount({ metrics, loading, error, revokingUserId, onRefresh, onRevoke }: { metrics?: AdminMetrics; loading: boolean; error: string; revokingUserId?: string; onRefresh: () => void; onRevoke: (userId: string, displayName: string) => void }) {
  return <section className="admin-session-count" aria-label="Administrator overview">
    <div className="admin-session-summary">
      <div className="admin-session-copy"><UsersRound/><span><strong>Signed-in people</strong><small>Unique accounts with a valid SYORA session.</small></span></div>
      <div className="admin-session-value"><strong aria-label={metrics ? `${metrics.signedInUsers} signed-in people` : 'Signed-in count unavailable'}>{metrics?.signedInUsers ?? '—'}</strong><button type="button" className="icon-button" aria-label="Refresh signed-in people" disabled={loading} onClick={onRefresh}>{loading ? <LoaderCircle className="spin"/> : <RefreshCw/>}</button></div>
    </div>
    {metrics?.people?.length ? <div className="admin-session-list" aria-label="Active accounts">{metrics.people.map(person => <div className="admin-session-person" key={person.userId}><span><strong>{person.displayName}{person.isCurrentUser && <em>You</em>}</strong><small>{usernameLabel(person.username)} · {person.sessionCount} {person.sessionCount === 1 ? 'session' : 'sessions'} · active {new Date(person.lastActiveAt).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</small></span>{person.canRevoke && <button type="button" className="button danger small" disabled={Boolean(revokingUserId)} onClick={() => onRevoke(person.userId, person.displayName)}>{revokingUserId === person.userId && <LoaderCircle className="spin"/>}<LogOut/> Log out</button>}</div>)}</div> : !loading && !error && <p className="muted-copy admin-session-empty">No valid sessions are active.</p>}
