...
export function Composer({conversationId,blocked,reply,clearReply}:{conversationId:string;blocked:boolean;reply?:Message;clearReply:()=>void}) {
 ...
 function resizeInput(element=input.current){if(!element)return;element.style.height='auto';element.style.height=Math.min(element.scrollHeight,140)+'px';}
 ...
 async function send(){
  ...
  services.typing(conversationId,false);setAttachment(undefined);setText('');clearReply();setEmoji(false);requestAnimationFrame(()=>resizeInput());
