...
import { Copy, Reply, Trash2, RotateCcw } from 'lucide-react';
...
export function MessageBubble({message,mine,original,onReply,groupedWithPrevious=false}:{message:Message;mine:boolean;original?:Message;onReply:()=>void;groupedWithPrevious?:boolean}) {
 ...
 return <article className={`message-row ${mine?'is-outgoing':'is-incoming'} ${groupedWithPrevious?'grouped':''}`} aria-label={mine?'Sent by you':'Received message'}>
  ...
  </div><div className="message-actions">{message.text&&!message.deleted&&<IconButton label={copied?'Message copied':'Copy message'} onClick={()=>void navigator.clipboard.writeText(message.text).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),1400)}).catch(()=>setError('Message could not be copied.'))}><Copy size={16}/></IconButton>}{!message.deleted&&<IconButton label="Reply to message" onClick={onReply}><Reply size={17}/></IconButton>}<IconButton label="Delete message" onClick={()=>setDeleting(true)}><Trash2 size={16}/></IconButton></div>
